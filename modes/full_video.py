...
Processes any-length video (including 4-5 min) and applies all
shorts/reels features as a single output:
  - 9:16 vertical fit with blurred background
  - Subtitle generation + burn-in
  - Text overlay
  - Thumbnail generation
  - Video modulation (hash-breaking)
"""
...
class FullVideoConfig:
    ...
    thumbnail_title: str = ""
    thumbnail_style: str = "bold"  # "bold" or "clean"

    # Modulation (hash-breaking)
    enable_modulation: bool = False
    modulation_preset: str = ""  # preset name or empty for custom
...
def _crop_to_vertical(video_path: str, output_path: str,
                      config: FullVideoConfig) -> bool:
    """Apply 9:16 scale-to-fit with blurred background (no content loss)."""
...
def process_full_video(video_path: str, output_dir: str,
                       config: Optional[FullVideoConfig] = None,
                       progress_callback=None) -> FullVideoResult:
    """
    Process a full-length video with all available post-production features.

    Pipeline (each step optional):
    1. Crop to 9:16 vertical
    2. Generate + burn subtitles
    3. Burn text overlay
    ...
    Args:
        ...
        config: FullVideoConfig (defaults if None)
        progress_callback: Optional fn(progress_pct, stage_msg)
    ...
    working_path = video_path  # current file being processed (chains through steps)
    ...
    try:
        ...
        if config.crop_vertical:
            ...
            if _crop_to_vertical(working_path, cropped_path, config):
                ...
                logger.info("Step 1: Vertical crop done")
            else:
                logger.warning("Vertical crop failed, continuing with original")
        ...
        if config.enable_subtitles:
            ...
            try:
                from post.subtitle_generator import generate_subtitles, burn_subtitles
        ...
        if config.enable_overlay and config.overlay_text:
            ...
            try:
                from post.subtitle_generator import burn_text_overlay
                ...
                logger.info("Step 3: Text overlay done")
            except Exception as e:
        ...
        if config.enable_thumbnail:
            ...
            try:
                from post.thumbnail_generator import generate_template_thumbnail
                ...
                logger.info("Step 5: Thumbnail generated")
            except Exception as e:
